<?php 
    ...
    $dates = $this->datos['dates'];
    $categories = $this->datos['categories'];
?>
...
<link rel="stylesheet" href="<?php echo constant('URL') ?>public/css/history.css">
    ...
    <?php $this->showMessages(); ?>
        <div id="history-container" class="container">
            <!-- Lo que se hace aqui es verificar si se recibe la accion de eliminar con exito o no  -->
            <?php 
                if(isset($_GET['message'])){
                    if($_GET['message'] == 'success'){
                        echo('Gasto eliminado con éxito');
                    }else{
                        echo('Hubo un error en la petición. Intentalo más tarde');
                    }
                }
            ?>
                ...
                <div id="filters-container">
                    <div class="filter-container">
                        <select id="sdate" class="custom-select">
                            ...
                            <!-- Solo se obtienen las fechas que tienen informacion -->
                            <?php
                                $options = $dates;
                                foreach($options as $option){
                                    echo "<option value=$option>".$option."</option></option>";
                                }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="filter-container">
                        <select id="scategory" class="custom-select">
                            ...
                            <!-- Solo se obtienen las categorias que tienen informacion -->
                            <?php
                                $options = $categories;
                                foreach($options as $option){
                                    echo "<option value=$option>".$option."</option></option>";
    ...
    <script>
        ...
        async function getData(){
            data = await fetch('http://crud-php.test/expenses/getHistoryJSON')
            .then(res =>res.json())
            .then(json => json);
            this.copydata = [...this.data];
            console.table(data);
        ...
        function renderData(data){
            ...
            data.forEach(item => { 
                ...
                databody.innerHTML += `<tr>
                        ...
                        <td>${item.date}</td>
                        <td>$${item.amount}</td>
                        <td><a href="http://crud-php.test/expenses/delete/${item.id}">Eliminar</a></td>
                    </tr>`;
            });
        }
